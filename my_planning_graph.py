#-----------------------------------------------------------------------------------------------
# This script is the solution for the third assignment (Part 2) of the Artificial Intelligence Nano degree
# program offered by Udacity. It is used to specify a Planning graph to be used for the 3 problems outlined
# in the script my_air_cargo_problems.py.
# 
# Author: Rahul Sharma
...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        """
        # This will add action A level to the planning graph as described in the Russell-Norvig text
        ...
        # Iterate through previous state levels to identify matching actions
        for s_node in self.s_levels[level]:
            for action in self.all_actions:
                ...
                elif s_node.symbol in action.precond_neg:
                    a_node = PgNode_a(action)
                    a_node.parents.add(s_node)
                    s_node.children.add(a_node)
                    self.a_levels[level].add(a_node)
    ...
    def add_literal_level(self, level):
        ...
        """
        # This will add literal S level to the planning graph as described in the Russell-Norvig text
        ...
        # Add state nodes from the previous action nodes into the state level
        for a_node in self.a_levels[level - 1]:
            s_nodes = a_node.effnodes
            for s_node in s_nodes:
                # Create links between state nodes and corresponding action nodes
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        snodes_a1 = node_a1.effnodes
        snodes_a2 = node_a2.effnodes

        # Iterate over the two nodes to verify any inconsistent effects
        for node_i in snodes_a1:
            for node_j in snodes_a2:
                if node_i.symbol == node_j.symbol and node_i.is_pos != node_j.is_pos:
                    return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        def check_negation(node_a1, node_a2):
            """
            Used to test a pair of actions for mutual exclusion

            :param node_a1: PgNode_a
            :param node_a2: PgNode_a
            :return :bool
            """
            eff = node_a1.effnodes
            precond = node_a2.prenodes
            for e in eff:
                for p in precond:
                    if e.symbol == p.symbol and e.is_pos != p.is_pos:
                        return True
            return False
        return check_negation(node_a1, node_a2) or check_negation(node_a2, node_a1)
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        snodes_a1 = node_a1.parents
        snodes_a2 = node_a2.parents
        for node_i in snodes_a1:
            for node_j in snodes_a2:
                if node_i.is_mutex(node_j):
                    return True
        return False
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        result = node_s1.symbol == node_s2.symbol and node_s1 != node_s2.is_pos
        return result
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        :return: bool
        """
        anodes_s1 = node_s1.parents
        anodes_s2 = node_s2.parents
        if not anodes_s1.isdisjoint(anodes_s2):
            return False
        for node_i in anodes_s1:
            for node_j in anodes_s2:
                if node_i.is_mutex(node_j):
                    return True
        return False
    ...
    def h_levelsum(self) -> int:
        ...
        for level in range(0, len(self.s_levels)):
            for state in self.s_levels[level]:
                if expr(state.symbol) in goals and state.is_pos:
                    level_sum += level
